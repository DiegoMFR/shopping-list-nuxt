export async function getProductsById(
  ...
  const selectById = (listId: string) =>
    db.prepare(`SELECT * FROM products WHERE id=${listId}`).first();

  const results = await Promise.all(
    products.map((product) => selectById(product))
  );
...
export async function getProductsByList(listId: string): Promise<ShoppingList> {
  ...
  const { results } = await db
    .prepare(`SELECT * FROM list_products WHERE list=${listId}`)
    .all();
  ...
  if (results) {
    const products = await getProductsById(
      results.map((row) => (row.product as string) ?? null)
    );
