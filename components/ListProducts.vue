<template>
    <div class="w-full">
        ...
        <section>
            ...
            <ShoppingListComp v-if="shoppingList.size" :list="Array.from(shoppingList)" @remove-item="handleAddToBasket" />
            <div
v-else
        ...
        <section>
            ...
            <ShoppingListComp
                v-if="removedList.size" :list="Array.from(removedList)" theme="deleted"
...
const addToList = async (item: ShoppingItem) => {
    await $fetch(`/api/products/add-product-to-list`, { method: 'POST', body: JSON.stringify({ productId: String(item.id), listId: String(listData.id) }) })
